...
.EXAMPLE
  ...
  .\rollout-host-permissions.ps1 -Credential $cred -InventoryPath .\hosts.example.csv
#>
[CmdletBinding(SupportsShouldProcess = $true)]
param(
    [Parameter(Mandatory = $true)]
    [pscredential]$Credential,
    [string]$InventoryPath = (Join-Path $PSScriptRoot "hosts.example.csv"),
...
foreach ($row in $rows) {
    ...
    if ($PSCmdlet.ShouldProcess($row.Host, "Apply HCC host permissions")) {
        ...
        try {
            ...
            Copy-Item -Path (Join-Path $ScriptRoot "config.example.psd1") -Destination "C:\Windows\Temp\" -ToSession $session -Force
            Invoke-Command -Session $session -ScriptBlock {
                param($Role)
                & "C:\Windows\Temp\setup-host-hcc-permissions.ps1" -HostRole $Role -ConfigPath "C:\Windows\Temp\config.example.psd1"
